...
class Tracker:
    ...
    def interpolate_ball_positions(self, ball_positions):
        ...
        df_ball_positions = df_ball_positions.interpolate(method='linear', limit_direction='both', axis=0)
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        for frame_num, detection in enumerate(detections):
            ...
            for frame_detection in detection_with_tracks:
                ...
                bytetrack_id = frame_detection[4]   # raw ByteTrack ID

                if cls_id == cls_names_inv['player']:
                    # Map ByteTrack ID → Stable ID
                    if bytetrack_id not in self.player_id_map:
                        self.player_id_map[bytetrack_id] = self.next_player_id
                        self.next_player_id += 1

                    stable_id = self.player_id_map[bytetrack_id]
